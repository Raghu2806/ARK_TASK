...
for y in range(height):
    for x in range(max_disp, width):
        ...
        for disp in range(max_disp):
            ...
            # Check if both windows are valid (non-empty) and have the same shape
            if left_window.size > 0 and right_window.size > 0 and left_window.shape == right_window.shape:
